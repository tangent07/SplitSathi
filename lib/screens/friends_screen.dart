                  ...
                  child: Container(
                    ...
                    child: const Center(
                      child: Text('👤 Add friend to your network', 
                        style: TextStyle(fontFamily: 'Inter', fontSize: 14, fontWeight: FontWeight.w800, color: AppColors.orange)
                      )
                    )
                  ),
                ),
  ...
  Widget _buildFriendRow(FriendData friend, String currency, bool isDark, Color textColor) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            Container(width: 38, height: 38, decoration: BoxDecoration(color: AppConstants.getAvatarColor(friend.name.length), borderRadius: BorderRadius.circular(12)), child: Center(child: Text(friend.name[0].toUpperCase(), style: const TextStyle(fontFamily: 'Inter', fontSize: 16, fontWeight: FontWeight.w900, color: Colors.white)))),
            const SizedBox(width: 12),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(friend.name, style: TextStyle(fontFamily: 'Inter', fontSize: 15, fontWeight: FontWeight.w700, color: textColor)), Text(friend.sharedGroups.isNotEmpty ? friend.sharedGroups.join(', ') : 'Direct', maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: 11, color: isDark ? AppColors.darkMuted : AppColors.muted))])),
            Text(balanceText, style: TextStyle(fontFamily: 'Inter', fontWeight: FontWeight.w800, fontSize: 13, color: balanceColor)),
          ],
        ),
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        leading: GestureDetector(onTap: () => Navigator.pop(context), child: const Row(children: [SizedBox(width: 16), Icon(Icons.arrow_back, size: 18), SizedBox(width: 4), Text('Back', style: TextStyle(fontFamily: 'Inter', fontWeight: FontWeight.w700, fontSize: 15))])),
        actions: [
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  Row(
                    children: [
                      Container(width: 64, height: 64, decoration: BoxDecoration(color: AppColors.orange, borderRadius: BorderRadius.circular(20)), child: Center(child: Text(widget.friend.name[0].toUpperCase(), style: const TextStyle(fontFamily: 'Inter', fontSize: 26, fontWeight: FontWeight.w900, color: Colors.white)))),
                      const SizedBox(width: 16),
                      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(widget.friend.name, style: TextStyle(fontFamily: 'Inter', fontSize: 28, fontWeight: FontWeight.w900, color: textColor)), Text(widget.friend.sharedGroups.isNotEmpty ? 'In: ${widget.friend.sharedGroups.join(', ')}' : 'Direct contact', style: TextStyle(fontSize: 13, color: isDark ? AppColors.darkMuted : AppColors.muted))])),
                    ],
                  ),
                  ...
                  Container(
                    ...
                    child: Column(children: [Text(cardTitle, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: cardText)), const SizedBox(height: 8), Text(cardAmount, style: TextStyle(fontFamily: 'Inter', fontSize: 44, fontWeight: FontWeight.w900, color: cardText, height: 1))]),
                  ),
                  const SizedBox(height: 24),
                  widget.friend.netBalance < -0.01 
                    ? GestureDetector(
                        onTap: _handleDirectSettleUp,
                        child: Container(width: double.infinity, padding: const EdgeInsets.symmetric(vertical: 16), decoration: BoxDecoration(color: AppColors.error, borderRadius: BorderRadius.circular(14)), child: const Center(child: Text('Settle Up (UPI / Cash) →', style: TextStyle(fontFamily: 'Inter', fontSize: 16, fontWeight: FontWeight.w800, color: Colors.white)))),
                      )
                    : GestureDetector(
                        ...
                        child: Container(width: double.infinity, padding: const EdgeInsets.symmetric(vertical: 16), decoration: BoxDecoration(color: AppColors.orange, borderRadius: BorderRadius.circular(14)), child: const Center(child: Text('+ Record Payment', style: TextStyle(fontFamily: 'Inter', fontSize: 16, fontWeight: FontWeight.w800, color: Colors.white)))),
                      ),
                  const SizedBox(height: 32),
                  Text('PAYMENT HISTORY', style: TextStyle(fontFamily: 'Inter', fontSize: 12, fontWeight: FontWeight.w800, color: AppColors.orange, letterSpacing: 1.5)),
                  const SizedBox(height: 12),
                ],
              ),
            ),
                ...
                builder: (context, directSnapshot) {
                  return FutureBuilder<List<Map<String,dynamic>>>(
                    ...
                    builder: (context, snapshot) {
                      ...
                      if (history.isEmpty) return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [const Text('💸', style: TextStyle(fontSize: 40)), const SizedBox(height: 12), Text('No history yet', style: TextStyle(fontFamily: 'Inter', fontSize: 16, fontWeight: FontWeight.w800, color: textColor))]));
                                  ...
                                  children: [
                                    ...
                                    Expanded(
                                      child: Column(
                                        crossAxisAlignment: CrossAxisAlignment.start, 
                                        children: [
                                          Text(item['title'], style: TextStyle(fontFamily: 'Inter', fontWeight: FontWeight.w800, fontSize: 15, color: textColor)), 
                                    ...
                                    Text('${isPos ? '+' : '-'} $currency${(item['amount'] as double).round()}', style: TextStyle(fontFamily: 'Inter', fontWeight: FontWeight.w900, fontSize: 16, color: isPos ? AppColors.success : AppColors.error)),
                                  ],
                                ),
                              ),
    ...
    return Container(
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            Center(child: Container(width: 40, height: 4, decoration: BoxDecoration(color: borderColor, borderRadius: BorderRadius.circular(2)))), const SizedBox(height: 16), Text('Record Payment', style: TextStyle(fontFamily: 'Inter', fontSize: 22, fontWeight: FontWeight.w900, color: textColor)), const SizedBox(height: 24), Text('WHO PAID?', style: TextStyle(fontFamily: 'Inter', fontSize: 12, fontWeight: FontWeight.w800, color: AppColors.orange, letterSpacing: 0.5)), const SizedBox(height: 8),
            Row(children: [Expanded(child: GestureDetector(onTap: () => setState(() => _youPaid = true), child: Container(padding: const EdgeInsets.symmetric(vertical: 14), decoration: BoxDecoration(color: _youPaid ? AppColors.orange.withOpacity(0.15) : inputBg, borderRadius: BorderRadius.circular(12), border: Border.all(color: _youPaid ? AppColors.orange : borderColor, width: 1.5)), child: Center(child: Text('💸 I paid them', style: TextStyle(fontFamily: 'Inter', fontWeight: FontWeight.w800, color: _youPaid ? AppColors.orange : (isDark ? AppColors.darkMuted : AppColors.muted))))))), const SizedBox(width: 12), Expanded(child: GestureDetector(onTap: () => setState(() => _youPaid = false), child: Container(padding: const EdgeInsets.symmetric(vertical: 14), decoration: BoxDecoration(color: !_youPaid ? AppColors.orange.withOpacity(0.15) : inputBg, borderRadius: BorderRadius.circular(12), border: Border.all(color: !_youPaid ? AppColors.orange : borderColor, width: 1.5)), child: Center(child: Text('🤲 They paid me', style: TextStyle(fontFamily: 'Inter', fontWeight: FontWeight.w800, color: !_youPaid ? AppColors.orange : (isDark ? AppColors.darkMuted : AppColors.muted)))))))]),
            const SizedBox(height: 20), Text('AMOUNT', style: TextStyle(fontFamily: 'Inter', fontSize: 12, fontWeight: FontWeight.w800, color: AppColors.orange, letterSpacing: 0.5)), const SizedBox(height: 8),
            TextField(controller: _amountController, keyboardType: const TextInputType.numberWithOptions(decimal: true), style: TextStyle(color: textColor, fontFamily: 'Inter', fontWeight: FontWeight.w800, fontSize: 18), decoration: InputDecoration(prefixIcon: Padding(padding: const EdgeInsets.all(14), child: Text(currency, style: const TextStyle(fontFamily: 'Inter', fontWeight: FontWeight.w900, fontSize: 18, color: AppColors.orange))), hintText: '0', hintStyle: TextStyle(color: isDark ? AppColors.darkMuted : AppColors.muted), filled: true, fillColor: inputBg, border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: borderColor)), focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: AppColors.orange, width: 1.5)))), const SizedBox(height: 20), Text('NOTE (OPTIONAL)', style: TextStyle(fontFamily: 'Inter', fontSize: 12, fontWeight: FontWeight.w800, color: AppColors.orange, letterSpacing: 0.5)), const SizedBox(height: 8),
            TextField(controller: _noteController, style: TextStyle(color: textColor, fontFamily: 'Inter', fontWeight: FontWeight.w700), decoration: InputDecoration(hintText: 'e.g. Borrowed for lunch...', hintStyle: TextStyle(color: isDark ? AppColors.darkMuted : AppColors.muted), filled: true, fillColor: inputBg, border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: borderColor)), focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: AppColors.orange, width: 1.5)))), const SizedBox(height: 24),
    ...
    return Container(
      ...
      child: SingleChildScrollView(
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min,
          children: [
            Center(child: Container(width: 40, height: 4, decoration: BoxDecoration(color: borderColor, borderRadius: BorderRadius.circular(2)))), const SizedBox(height: 16), Text('New Payment', style: TextStyle(fontFamily: 'Inter', fontSize: 22, fontWeight: FontWeight.w900, color: textColor)), const SizedBox(height: 24), Text('FRIEND\'S NAME', style: TextStyle(fontFamily: 'Inter', fontSize: 12, fontWeight: FontWeight.w800, color: AppColors.orange, letterSpacing: 0.5)), const SizedBox(height: 8),
            TextField(controller: _nameController, style: TextStyle(color: textColor, fontFamily: 'Inter', fontWeight: FontWeight.w700), decoration: InputDecoration(hintText: 'Enter name...', hintStyle: TextStyle(color: isDark ? AppColors.darkMuted : AppColors.muted), filled: true, fillColor: inputBg, border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: borderColor)), focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: AppColors.orange, width: 1.5)))), const SizedBox(height: 20), Text('WHO PAID?', style: TextStyle(fontFamily: 'Inter', fontSize: 12, fontWeight: FontWeight.w800, color: AppColors.orange, letterSpacing: 0.5)), const SizedBox(height: 8),
            Row(children: [Expanded(child: GestureDetector(onTap: () => setState(() => _youPaid = true), child: Container(padding: const EdgeInsets.symmetric(vertical: 14), decoration: BoxDecoration(color: _youPaid ? AppColors.orange.withOpacity(0.15) : inputBg, borderRadius: BorderRadius.circular(12), border: Border.all(color: _youPaid ? AppColors.orange : borderColor, width: 1.5)), child: Center(child: Text('💸 I paid them', style: TextStyle(fontFamily: 'Inter', fontWeight: FontWeight.w800, color: _youPaid ? AppColors.orange : (isDark ? AppColors.darkMuted : AppColors.muted))))))), const SizedBox(width: 12), Expanded(child: GestureDetector(onTap: () => setState(() => _youPaid = false), child: Container(padding: const EdgeInsets.symmetric(vertical: 14), decoration: BoxDecoration(color: !_youPaid ? AppColors.orange.withOpacity(0.15) : inputBg, borderRadius: BorderRadius.circular(12), border: Border.all(color: !_youPaid ? AppColors.orange : borderColor, width: 1.5)), child: Center(child: Text('🤲 They paid me', style: TextStyle(fontFamily: 'Inter', fontWeight: FontWeight.w800, color: !_youPaid ? AppColors.orange : (isDark ? AppColors.darkMuted : AppColors.muted)))))))]),
            const SizedBox(height: 20), Text('AMOUNT', style: TextStyle(fontFamily: 'Inter', fontSize: 12, fontWeight: FontWeight.w800, color: AppColors.orange, letterSpacing: 0.5)), const SizedBox(height: 8),
            TextField(controller: _amountController, keyboardType: const TextInputType.numberWithOptions(decimal: true), style: TextStyle(color: textColor, fontFamily: 'Inter', fontWeight: FontWeight.w800, fontSize: 18), decoration: InputDecoration(prefixIcon: Padding(padding: const EdgeInsets.all(14), child: Text(currency, style: const TextStyle(fontFamily: 'Inter', fontWeight: FontWeight.w900, fontSize: 18, color: AppColors.orange))), hintText: '0', hintStyle: TextStyle(color: isDark ? AppColors.darkMuted : AppColors.muted), filled: true, fillColor: inputBg, border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: borderColor)), focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: AppColors.orange, width: 1.5)))), const SizedBox(height: 20), Text('NOTE (OPTIONAL)', style: TextStyle(fontFamily: 'Inter', fontSize: 12, fontWeight: FontWeight.w800, color: AppColors.orange, letterSpacing: 0.5)), const SizedBox(height: 8),
            TextField(controller: _noteController, style: TextStyle(color: textColor, fontFamily: 'Inter', fontWeight: FontWeight.w700), decoration: InputDecoration(hintText: 'e.g. Borrowed for lunch...', hintStyle: TextStyle(color: isDark ? AppColors.darkMuted : AppColors.muted), filled: true, fillColor: inputBg, border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: borderColor)), focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: AppColors.orange, width: 1.5)))), const SizedBox(height: 24),
