...
import 'package:url_launcher/url_launcher.dart'; // <-- NEW WHATSAPP PACKAGE
...
class _GroupDetailScreenState extends State<GroupDetailScreen>
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      Expanded(child: _balanceCard('YOU OWE', '$currency${myOwe.round()}', AppColors.error, surfaceColor, borderColor)),
                      const SizedBox(width: 12),
                      Expanded(child: _balanceCard('YOU GET BACK', '$currency${myReceive.round()}', AppColors.success, surfaceColor, borderColor)),
                    ],
                  ),
                  const SizedBox(height: 16),
                  Container(
                    ...
                    child: TabBar(
                      ...
                      indicator: BoxDecoration(color: surfaceColor, borderRadius: BorderRadius.circular(9)),
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          ...
          showModalBottomSheet(
            ...
            builder: (_) => AddExpenseSheet(groupId: widget.groupId, members: group.members),
          );
        },
  ...
  Widget _balanceCard(String label, String amount, Color amountColor, Color surface, Color border) {
    return Container(
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        if (index > 0) {
          ...
          prevDate = DateFormat('d MMM yyyy').format(prev.deleted ? (prev.deletedAt ?? prev.date) : prev.date);
        }
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(child: Text(
                      ...
                      style: TextStyle(fontStyle: FontStyle.italic, fontSize: 13, color: isDark ? AppColors.darkMuted : AppColors.muted),
                    )),
                    Text(DateFormat('hh:mm a').format(exp.deletedAt ?? exp.date), style: TextStyle(fontSize: 10, color: isDark ? AppColors.darkMuted : AppColors.muted)),
                  ],
                ),
              )
            else
                  ...
                  child: Container(
                    ...
                    child: Row(
                      children: [
                        Container(
                          width: 44, height: 44,
                          decoration: BoxDecoration(color: isDark ? AppColors.darkSurface2 : AppColors.peach, borderRadius: BorderRadius.circular(12)),
  ...
  Widget build(BuildContext context) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, i) {
        ...
        final color = bal > 0 ? AppColors.success : bal < 0 ? AppColors.error : isDark ? AppColors.darkMuted : AppColors.muted;
        final label = bal > 0 ? 'gets back $currency$bal' : bal < 0 ? 'owes $currency${bal.abs()}' : 'settled up ✓';
...
class _SettleUpTabState extends State<_SettleUpTab> {
  ...
  void _markFullSettled(int i, String currency) async {
    ...
    await DatabaseService().addSettlement(
      widget.group.id, '$from paid $to', amount, from, to, '✅', '$from settled $currency${amount.round()} → $to'
    );
  ...
  void _markPartialSettled(int i, String currency) async {
    ...
    await DatabaseService().addSettlement(
      widget.group.id, '$from partially paid $to', partial, from, to, '🔄', '$from partially paid $currency${partial.round()} → $to'
    );
  ...
  // --- THE NEW WHATSAPP FUNCTION ---
  void _shareWhatsApp(String currency) async {
    String msg = '💰 *${widget.group.name} - SplitSathi Summary*\n\n';
    for (var t in widget.balances) {
      msg += '• ${t['from']} needs to pay ${t['to']}: $currency${(t['amount'] as double).round()}\n';
    }
    msg += '\n_Sent via SplitSathi_ 🤝';
...
// ── PARTIAL NUMPAD ─────────────────────────────────────────────────
// (Unchanged Numpad code goes here, omitted for brevity but keep it in your file!)
...
class _PartialNumpadState extends State<_PartialNumpad> {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      decoration: BoxDecoration(color: bg, borderRadius: const BorderRadius.vertical(top: Radius.circular(24)), border: const Border(top: BorderSide(color: AppColors.orange, width: 2))),
      padding: const EdgeInsets.fromLTRB(16, 20, 16, 32),
      child: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          Text('Max: $currency${widget.max}', style: TextStyle(fontSize: 12, color: isDark ? AppColors.darkMuted : AppColors.muted)),
          const SizedBox(height: 8),
          Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
          ...
          const SizedBox(height: 16),
          GridView.count(
            shrinkWrap: true, crossAxisCount: 3, mainAxisSpacing: 10, crossAxisSpacing: 10, childAspectRatio: 2, physics: const NeverScrollableScrollPhysics(),
            children: [
              ...['1','2','3','4','5','6','7','8','9','.','0'].map((k) => _key(k, keyBg, borderColor, textColor, () => _press(k))),
              _key('⌫', keyBg, borderColor, AppColors.error, _delete),
            ],
          ),
          const SizedBox(height: 12),
          GestureDetector(
            onTap: () { widget.onConfirm(_value == '0' ? '' : _value); Navigator.pop(context); },
            child: Container(
...
class _EditExpenseSheetState extends State<_EditExpenseSheet> {
  ...
  void _toggleMember(String member) {
    setState(() {
      if (_splitAmong.contains(member)) { if (_splitAmong.length > 1) _splitAmong.remove(member); } 
      else { _splitAmong.add(member); }
    });
  }
  ...
  void _openNumpad() {
    showModalBottomSheet(
      ...
      builder: (_) => _PartialNumpad(initial: _amountController.text, max: 9999999, onConfirm: (val) => setState(() => _amountController.text = val)),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    final share = _splitAmong.isEmpty ? 0.0 : (double.tryParse(_amountController.text) ?? 0) / _splitAmong.length;
            ...
            Row(
              ...
              children: AppConstants.expenseCategories.map((cat) {
                ...
                return GestureDetector(
                  ...
                  child: Container(
                    width: 50, height: 50,
                    decoration: BoxDecoration(color: selected ? AppColors.orange.withOpacity(0.15) : inputBg, borderRadius: BorderRadius.circular(12), border: Border.all(color: selected ? AppColors.orange : borderColor, width: selected ? 2 : 1.5)),
